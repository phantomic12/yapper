...
- E2E OCR tests (Playwright) require a running preview server:
  ```bash
  npm run build && npx vite preview --port 4180
  E2E_BASE_URL=http://localhost:4180 node scripts/e2e-ocr-test.mjs
  ```
  Test PDFs/docs are generated by `scripts/gen_test_pdfs.py` and
  `scripts/gen_test_docs.py` (requires Python + `python-docx`).
